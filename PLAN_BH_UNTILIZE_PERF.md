...
2. Defer destination-programming hoist/reuse across repeated same-width chunks.
   It was prototyped in `dd919304c32` and then reverted because the default-safe
   gate only helped `ct>=16` by ~1.8% L1_TO_L1 while adding row-begin/current/
   advance API complexity. Reconsider only if production coverage shows common
   `ct>=16` fast-untilize rows or if a later pack-loop refactor makes the helper
   surface natural.
