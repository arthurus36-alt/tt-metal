...
// Metal 2.0 mirror of extract_tensor_buffers_t: walks the same Tensor aggregates
// but pushes std::cref(t.mesh_tensor()) instead of t.buffer().  Generated per-T
// at compile time so the cache-hit hot path avoids runtime reflection visitor
// dispatch and the per-call vtable-like indirection it implies.
//
// Sharing the walk machinery with extract_tensor_buffers_t would be nicer (one
// shared template parameterised on a leaf functor); deferred to a follow-up so
// this PR stays scoped to perf parity.
template <typename T, typename = void>
struct extract_mesh_tensors_t {
    template <typename Out>
    static void call(const T&, Out&) {}
};

template <typename T, typename Out>
inline void extract_mesh_tensors_into(const T& obj, Out& out) {
    extract_mesh_tensors_t<std::decay_t<T>>::call(obj, out);
}

// Tensor leaf — push the MeshTensor reference.
template <>
struct extract_mesh_tensors_t<tt::tt_metal::Tensor, void> {
    template <typename Out>
    static void call(const tt::tt_metal::Tensor& t, Out& out) {
        out.push_back(std::cref(t.mesh_tensor()));
    }
};
...
struct extract_mesh_tensors_t<
    ...
    template <typename Out>
    static void call(const T& obj, Out& out) {
        reflect::for_each([&obj, &out](auto I) { extract_mesh_tensors_into(reflect::get<I>(obj), out); }, obj);
    }
};

// Optional — visit value if present.
template <typename T>
struct extract_mesh_tensors_t<std::optional<T>, void> {
    template <typename Out>
    static void call(const std::optional<T>& v, Out& out) {
        if (v.has_value()) {
            extract_mesh_tensors_into(v.value(), out);
        }
    }
};
...
// Vector — runtime loop (unavoidable, count is dynamic).
template <typename T>
struct extract_mesh_tensors_t<std::vector<T>, void> {
    template <typename Out>
    static void call(const std::vector<T>& v, Out& out) {
        for (const auto& e : v) {
            extract_mesh_tensors_into(e, out);
        }
    }
};
...
// Array — unroll at compile time.
template <typename T, std::size_t N>
struct extract_mesh_tensors_t<std::array<T, N>, void> {
    template <typename Out>
    static void call(const std::array<T, N>& v, Out& out) {
        [&]<std::size_t... Is>(std::index_sequence<Is...>) {
            (extract_mesh_tensors_into(v[Is], out), ...);
        }(std::make_index_sequence<N>{});
    }
};
...
// Tuple — unroll at compile time.
template <typename... Ts>
struct extract_mesh_tensors_t<std::tuple<Ts...>, void> {
    template <typename Out>
    static void call(const std::tuple<Ts...>& v, Out& out) {
        [&]<std::size_t... Is>(std::index_sequence<Is...>) {
            (extract_mesh_tensors_into(std::get<Is>(v), out), ...);
        }(std::make_index_sequence<sizeof...(Ts)>{});
    }
};
...
public:
    ...
    struct Metal2MeshWorkloadFactoryAdapter {
        ...
        // Per-program cache state. `tensor_args_storage` is sized once at cache
        // miss and mutated in place on every cache hit so the hot path performs
        // zero heap allocation. Same precedent as the descriptor adapter's
        // ResolvedBindings + SmallVector pattern.
        struct shared_variables_t {
            std::vector<ResolvedTensorBinding> bindings;
            std::vector<TensorArg> tensor_args_storage;
        };
        using cached_mesh_workload_t = AdaptedCachedMeshWorkload<shared_variables_t>;

        using MeshTensorRefVec = ttsl::SmallVector<std::reference_wrapper<const tt::tt_metal::MeshTensor>, 16>;

        // Walk tensor_args and tensor_return_value via the compile-time-unrolled
        // extract_mesh_tensors_t template, collecting the MeshTensor reference of
        // every Tensor leaf. The walk order is deterministic so the resulting
        // indices are stable across calls. Stack-allocated SmallVector (16 inline
        // slots) so the cache-hit fast path avoids the per-dispatch allocation
        // tax. Metal 2.0 analog of the descriptor adapter's collect_tensor_buffers.
        static MeshTensorRefVec collect_mesh_tensors(
            const tensor_args_t& tensor_args, const tensor_return_value_t& tensor_return_value) {
            MeshTensorRefVec result;
            extract_mesh_tensors_into(tensor_args, result);
            extract_mesh_tensors_into(tensor_return_value, result);
            return result;
        }
        ...
        // Cache-miss path only; not on the hot path.
        static std::vector<ResolvedTensorBinding> resolve_bindings(
            const std::vector<TensorArg>& factory_tensor_args, const MeshTensorRefVec& io_mesh_tensors) {
            std::vector<ResolvedTensorBinding> bindings;
        ...
        static auto create_mesh_workload(
            ...
            for (auto& [range, prog_artifacts] : artifacts.programs) {
                ...
                // Pre-seed the reusable TensorArg storage with the factory's
                // initial values; cache-hit path mutates only the `.tensor` field.
                std::vector<TensorArg> tensor_args_storage(
                    prog_artifacts.run_params.tensor_args.begin(), prog_artifacts.run_params.tensor_args.end());

                shared_variables.emplace(
                    range,
                    shared_variables_t{
                        .bindings = std::move(bindings), .tensor_args_storage = std::move(tensor_args_storage)});
        ...
        static void apply_descriptor(
            ...
            for (auto& [coordinate_range, program] : cached_workload.workload.get_programs()) {
                auto& sv = cached_workload.shared_variables.at(coordinate_range);
                // Mutate the cached storage in place — no allocation on the hot path.
                for (std::size_t i = 0; i < sv.bindings.size(); ++i) {
                    sv.tensor_args_storage[i].tensor = io_mesh_tensors[sv.bindings[i].io_tensor_idx];
                }
                tt::tt_metal::experimental::metal2_host_api::UpdateTensorArgs(program, sv.tensor_args_storage);
            }
        }
    };
