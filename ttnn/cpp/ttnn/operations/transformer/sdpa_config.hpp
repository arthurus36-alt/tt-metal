...
struct SDPAProgramConfig {
    ...
    // (false) keeps the hierarchical parallelization. Supports causal and non-causal paths,
    // chunked prefill (legacy + flexible), and attention_sink. When causal with an even
    // q_num_chunks, a zigzag remap pairs light/heavy q_chunks per core for load balancing.
