...
void kernel_main() {
    ...
    // Global Q scheduling args at slots 11..12 — past the max phase_2 slot (10), so no
    ...
    if constexpr (global_q_scheduling) {
        global_q_start = get_arg_val<uint32_t>(11);
        global_q_count = get_arg_val<uint32_t>(12);
    }
