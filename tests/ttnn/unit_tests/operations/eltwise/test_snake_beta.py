...
def _sample_clamped_normal(shape, mean, std, lo, hi):
    t = torch.randn(shape, dtype=torch.float32) * std + mean
    return t.clamp_(lo, hi).to(torch.bfloat16)


@pytest.mark.parametrize("preset", ["c1", "c2", "c3"])
def test_snake_beta_real_workload(device, preset):
    """Bf16 real-workload distributions from issue #43337; asserts <= 2 ULP."""
    torch.manual_seed(0)
    shape, xm, xs, xlo, xhi, am, as_, alo, ahi, bm, bs, blo, bhi = _REAL_WORKLOAD_PRESETS[preset]
    D = shape[-1]

    x = _sample_clamped_normal(shape, xm, xs, xlo, xhi)
    a = _sample_clamped_normal((D,), am, as_, alo, ahi)
    b = _sample_clamped_normal((D,), bm, bs, blo, bhi)
    expected = torch_snake_beta(x, a, b)
