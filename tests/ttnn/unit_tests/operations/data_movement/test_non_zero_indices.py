...
def reference_flat_nonzero(torch_tensor):
    """Return flat (linear) indices of non-zero elements, matching kernel output."""
    return torch.nonzero(torch_tensor.flatten(), as_tuple=True)[0].int()


def run_nonzero_and_validate(torch_input, ttnn_input, device):
    """Run ttnn.nonzero, extract count + indices, compare to torch reference."""
    output = ttnn.nonzero(ttnn_input)

    # output_0: count tensor [1, 1, 1, 8]
    count_tensor = ttnn.to_torch(ttnn.from_device(output[0]))
    count = count_tensor[0, 0, 0, 0].item()

    # output_1: flat indices [1, 1, 1, N] where N = volume of input
    indices_tensor = ttnn.to_torch(ttnn.from_device(output[1]))
    tt_indices = indices_tensor[0, 0, 0, :count].int()

    ref_indices = reference_flat_nonzero(torch_input)

    assert count == ref_indices.numel(), f"Count mismatch: got {count}, expected {ref_indices.numel()}"
    assert_equal(tt_indices.unsqueeze(0).unsqueeze(0).unsqueeze(0), ref_indices.unsqueeze(0).unsqueeze(0).unsqueeze(0))
...
def test_nonzero_backward_compat_1d(size, device):
    """1D-like [1, 1, 1, X] ROW_MAJOR INTERLEAVED: output must be unchanged."""
    torch.manual_seed(0)
    ...
    count = count_tensor[0, 0, 0, 0].item()
    indices_tensor = ttnn.to_torch(ttnn.from_device(output[1]))
    tt_indices = indices_tensor[0, 0, 0, :count]

    # Reference: flat indices via flattened nonzero (matches kernel output format)
    ref_indices = reference_flat_nonzero(torch_input).float()
    assert count == ref_indices.numel()
    assert_equal(
        tt_indices.unsqueeze(0).unsqueeze(0).unsqueeze(0).float(),
        ref_indices.unsqueeze(0).unsqueeze(0).unsqueeze(0),
    )
