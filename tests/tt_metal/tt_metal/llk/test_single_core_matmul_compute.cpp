...
TEST_F(LLKBlackholeSingleCardFixture, TensixTestSingleCoreSingleBlockSingleTileComputeMatmulFp8e4m3Fp16Dest) {
    ASSERT_TRUE(unit_tests::compute::matmul::single_block_matmul(
        this->devices_.at(0),
        1,
        1,
...
// Mixed-family matmul: B-family input A x A-family-promoted Fp8 input B.
// Exercises Table 2 † rows from the FP8 enablement design.  After
// llk_unpack_hw_configure's in1/in0 swap in compute/matmul.h, this maps to
// SrcA = Fp8 (A-family promotion), SrcB = Float16_b / Bfp8_b (B-family).
//
// Verified on bh-34-special: FAILS on hardware in both fp32_dest modes.
//   fp32_dest=false: device output all zeros (pack underflow signature).
//   fp32_dest=true:  device output ~2^112x the golden magnitude
//                    (= Float16_b bias 127 - Float16 bias 15 = 112).
// Indicates an FPU-level mishandling of mixed A/B-family SrcA/SrcB rather
// than a JIT/cpack/cunpack family-tag issue: even Float32 DEST is wrong.
// Disabled pending LLK / hardware investigation; re-enable when supported.
TEST_F(LLKBlackholeSingleCardFixture, DISABLED_TensixTestSingleCoreSingleTileComputeMatmulFloat16bAFp8B) {
    ASSERT_TRUE(unit_tests::compute::matmul::single_tile_matmul(
        this->devices_.at(0),
        tt::DataFormat::Float16_b,
        tt::DataFormat::Fp8_e4m3,
...
TEST_F(LLKBlackholeSingleCardFixture, DISABLED_TensixTestSingleCoreSingleTileComputeMatmulBfp8bAFp8B) {
    ASSERT_TRUE(unit_tests::compute::matmul::single_tile_matmul(
        this->devices_.at(0),
...
TEST_F(LLKBlackholeSingleCardFixture, DISABLED_TensixTestSingleCoreSingleTileComputeMatmulFloat16bAFp8BFp32Dest) {
    ASSERT_TRUE(unit_tests::compute::matmul::single_tile_matmul(
        this->devices_.at(0),
        tt::DataFormat::Float16_b,
        tt::DataFormat::Fp8_e4m3,
...
TEST_F(LLKBlackholeSingleCardFixture, DISABLED_TensixTestSingleCoreSingleTileComputeMatmulBfp8bAFp8BFp32Dest) {
    ASSERT_TRUE(unit_tests::compute::matmul::single_tile_matmul(
        this->devices_.at(0),
        tt::DataFormat::Bfp8_b,
        tt::DataFormat::Fp8_e4m3,
